...
model3 <- coxph(Surv(duryear,status)~mpdummy+majordummy+loggdp1+loggpc10+gg100
                +mgr100+opec+ethnic100+region,data=military)
summary(model3)
ggforest(model3, data=military)

model4 <- coxph(Surv(duryear,status)~mpdummy+majordummy+loggdp1+loggpc10+gg100, 
                data = military)
summary(model4)

model5 <- coxph(Surv(duryear,status)~mpdummy+majordummy+mgr100,data=military)
summary(model5)

model6 <- coxph(Surv(duryear,status)~mpdummy+majordummy+region,data=military)
summary(model6)

model7 <- coxph(Surv(duryear,status)~mpdummy+majordummy+ethnic100,data=military)
summary(model7)

model8 <- coxph(Surv(duryear,status)~mpdummy+majordummy+opec,data=military)
summary(model8)

model9 <- coxph(Surv(duryear,status)~mpdummy+majordummy,data=military)
summary(model9)

stargazer(model9,model4,model5,model8,model7,model6,model3, type="html", 
          digits=1, out="table1.doc",flip=TRUE)

# 4. linear regression duration and party
duration1 <- lm(duryear~mpdummy+majordummy+loggdp1+loggpc10+gg100+mgr100+opec+
                        region+ethnic100,data=military)
car::vif(duration1)
sqrt(car::vif(duration1)) > 2
summary(duration1)

duration2 <- lm(duryear~mpdummy+majordummy+loggdp1+loggpc10+gg100,data=military)
car::vif(duration2)
...
duration4 <- lm(duryear~mpdummy+majordummy+opec, data = military)
car::vif(duration4)
sqrt(car::vif(duration4)) > 2
summary(duration4)

duration5 <- lm(duryear~mpdummy+majordummy+region, data = military)
car::vif(duration5)
sqrt(car::vif(duration5)) > 2
summary(duration5)

duration6 <- lm(duryear~mpdummy+majordummy+ethnic100, data = military)
car::vif(duration6)
sqrt(car::vif(duration6)) > 2
summary(duration6)

duration7 <- lm(duryear~mpdummy+majordummy, data = military)
car::vif(duration7)
sqrt(car::vif(duration7)) > 2
summary(duration7)

stargazer(duration7,duration2,duration3,duration4,duration6,duration5,duration1,
          type="html", digits=1, out="table2.doc", flip=TRUE)
...
seat1 <- lm(seat100_1~yeardiff+seat100_2+opposnum+timediff+loggdp1+loggpc10+
                    gg100+mgr100+opec+ethnic100,data=military)
car::vif(seat1)
sqrt(car::vif(seat1)) > 2
summary(seat1)

seat2 <- lm(seat100_1~yeardiff+seat100_2+opposnum+loggdp1+loggpc10+gg100,
            data=military)
car::vif(seat2)
sqrt(car::vif(seat2)) > 2
summary(seat2)

seat3 <- lm(seat100_1~yeardiff+seat100_2+opposnum+mgr100,data=military)
car::vif(seat3)
sqrt(car::vif(seat3)) > 2
summary(seat3)

seat4 <- lm(seat100_1~yeardiff+seat100_2+opposnum+opec,data=military)
car::vif(seat4)
sqrt(car::vif(seat4)) > 2
summary(seat4)

seat5 <- lm(seat100_1~yeardiff+seat100_2+opposnum+ethnic100,data=military)
car::vif(seat5)
sqrt(car::vif(seat5)) > 2
summary(seat5)

seat6 <- lm(seat100_1~yeardiff+seat100_2+opposnum+timediff,data=military)
car::vif(seat6)
sqrt(car::vif(seat6)) > 2
summary(seat6)

seat7 <- lm(seat100_1~yeardiff+seat100_2+opposnum,data=military)
car::vif(seat7)
sqrt(car::vif(seat7)) > 2
summary(seat7)

stargazer(seat7,seat2,seat3,seat4,seat5,seat6,seat1, type="html", digits=1, 
          out="table3.doc", flip=TRUE)
...
stargazer(military2, type = "html", 
          summary.stat = c("min", "median", "max", "sd", "mean","n"), 
